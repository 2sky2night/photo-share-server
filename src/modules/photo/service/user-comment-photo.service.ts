...
export class UserCommentPhotoService {
   ...
   * @returns 评论列表
   */
  async getComments(pid: number, offset: number, limit: number, currentUid: number | undefined) {
    const { rows, count: total } = await this.UCPModel.findAndCountAll({ where: { pid }, offset, limit })
    // 当前登录的用户
   ...
   * @returns 详情信息列表
   */
  async getCommentsInfo(comments: UserCommentPhoto[], currentUid: number | undefined) {
    return await Promise.all(comments.map(async (comment) => {
      ...
      // 查询评论的作者
      const user = await this.userService.findUserWithoutPasswordAndRole(comment.uid)
      return {
        ...comment.dataValues,
        like_count,
        is_liked,
